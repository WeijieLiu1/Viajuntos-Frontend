...
class MapWidget extends StatefulWidget {
  ...
  bool isEvent;
  MapWidget(
      {Key? key, required this.lat, required this.long, required this.isEvent})
      : super(key: key);
                        ...
                        MarkerLayerOptions(
                          markers: [
                            ...
                            for (var i = 0; i < stations.length; i++)
                              stations[i]["lat"] != null &&
                                      stations[i]["long"] != null &&
                                      stations[i]["pollution"] != null
                                  ? Marker(
                                      width: 35.0,
                                      height: 35.0,
                                      point: LatLng(stations[i]["lat"],
                                          stations[i]["long"]),
                                      builder: (context) => IconButton(
                                          icon: const Icon(Icons.device_thermostat,
                                              size: 30),
                                          onPressed: () =>
                                              showStation(stations[i]["id"]),
                                          color: Color.lerp(
                                              Colors.green,
                                              Color.lerp(
                                                  Colors.yellow,
                                                  ...
                                                  stations[i]['pollution'] < 0.15
                                                      ...
                                                      : (stations[i]['pollution'] > 0.3
                                                          ...
                                                          : (stations[i]['pollution'] - 0.15) /
                                                              0.15)),
                                              stations[i]['pollution'] > 0.15 ? 1 : stations[i]['pollution'] / 0.15)))
                                  : Marker(point: LatLng(0.0, 0.0), builder: (context) => const SizedBox())
                          ],
                        ),
                      ],
                    );
